...
def check_gpus():
    ...
    if not torch.cuda.is_available():
        print('This script could only be used to manage NVIDIA GPUs, but not GPU found in your device!')
        return False
    elif not 'NVIDIA System Management' in os.popen('nvidia-smi -h').read():
        print("The 'nvidia-smi' tool not found.")
        return False
    return True
